...
public class MeuTST<Value extends Comparable<Value>> {
	...
	private KeyVal(String key, Value val){
	    this.key = key;
	    this.val = val;
	}
	...
	public int compareTo(KeyVal outro) {
	    return outro.getVal().compareTo(this.getVal()); //para que na queue a ordem dos valores seja decrescente.
	}
	...
	if (prefix == null) {
        ...
        Iterable<String> list = keysWithPrefix(prefix); //lista com todas as keys com o prefixo
	ArrayList<KeyVal> array = new ArrayList<KeyVal>(); 
	for(String key: list) {
	    KeyVal keyval = new KeyVal(key, get(key)); //cria o objeto KeyVal e o adiciona ao ArrayList
	    array.add(keyval);	    
	}
	
	Collections.sort(array); //sort pelo metodo compareTo declarado na classe KeyVal
	Queue<String> queue = new Queue<String>(); //Queue a ser retornada
	for (int j = 0; j < array.size(); j++)
	    queue.enqueue(array.get(j).getKey());
        return queue;
    }
	...
	if (key == null) {
            throw new NullPointerException("calls delete() with null argument");
	}
        put(key, null);
	n--;
    }
    */
    ...
    public static void main(String[] args) {
	    ...
	    if (prefix.compareTo("aaa")==0) break;
