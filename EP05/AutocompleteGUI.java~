 ...
 *  Dependencies: Autocomplete.java Term.java (MeuTST.java em MAC0323)
 *    
...
/*******************************************************************************
 * Esta classe foi originalmente escrita para o terceiro Assignement de COS226
 * da ediÃ§Ã£o 2017.
 * 
 * Esta Ã© uma adaptaÃ§Ã£o que substitui as classse Term e BinarySearchDeluxe originais 
 * do assignment por uma adaptaÃ§Ã£o da classe TST.
 * 
 * As linhas adaptadas para MAC0323 contÃ©m um comentÃ¡rio com o rÃ³tulo "MAC0323"
 * (hmm, quase todas...)
 */

// import edu.princeton.cs.algs4.StdOut; // MAC0323 Usado para achar erros na adaptaÃ§Ã£o
...
public class AutocompleteGUI extends JFrame {
    ...
    public AutocompleteGUI(String filename, int k) {
        ...
        setTitle("Autocomplete Me de COS226 adaptado para MAC0323");
    ...
    private class AutocompletePanel extends JPanel {
        ...
        // private Autocomplete auto;             // the Autocomplete object MAC0323
        private MeuTST<Long> auto;                   // the MeuTST object MAC0323
        ...
        public AutocompletePanel(String filename) {
            ...
            try {
                ...
                // terms = new Term[n];
                // Initializes an empty string symbol table. MAC0323
                auto = new MeuTST<Long>(); // MAC0323
                for (int i = 0; i < n; i++) {
                    ...
                    // terms[i] = new Term(query, weight); MAC0323
                    auto.put(query, weight); // MAC0323
                }
            }
            catch (Exception e) {
            ...
            // auto = new Autocomplete(terms); MAC0323
            // auto passou a ser uma tabela de sÃ­mbolos da classe MeuTST
            ...
            searchText.getDocument().addDocumentListener(
                    new DocumentListener() {
                        ...
                        {
                            String text = searchText.getText();
                            // StdOut.println("text = " + text); MAC0323
                            // updates the drop-down menu
                            getSuggestions(text);
                            // StdOut.println("Voltei"); MAC0323
                            updateListSize();
                        }
                    });
        ...
        public void getSuggestions(String text) {
            ...
            if (text.equals("")) {
                ...
                // Term[] allResults = auto.allMatches(text); MAC0323
                // MAC0323: Returns all of the keys in the set that start with prefix.
                // Iterable<String> allResults = auto.keysWithPrefix(text); // MAC0323
                ...
                // StdOut.println("cheguei atÃ© aqui: " + text + "  " + textLen); MAC0323
                
                // results = new String[Math.min(k, allResults.length)]; MAC0323
                // if (Math.min(k, allResults.length) > 0) { MAC0323
                //    for (int i = 0; i < results.length; i++) { MAC0323
                int allResultsSize = 0; // MAC0323
                for (String query: allResults) { // MAC0323
                    allResultsSize += 1;         // MAC0323 NÃ£o sei pegar o size de Iterable
                    // StdOut.println(query + auto.get(query)); MAC0323
                }

                int resultsSize = Math.min(k, allResultsSize); // MAC0323
                // StdOut.println("Results Size = " + resultsSize); MAC0323

                // results = new String[Math.min(k, allResults.length)]; MAC0323
                results = new String[resultsSize]; // MAC0323
                // if (Math.min(k, allResults.length) > 0) { MAC0323
                if (resultsSize > 0) { // MAC0323
                    // for (int i = 0; i < results.length; i++) { MAC0323
                    int i = 0;
                    for (String term : allResults) { // MAC0323
                        String query = term;
                        
                        // A bit of a hack to get the Term's query string 
                        // and weight from toString()
                        // String next = allResults[i].toString(); MAC0323
                        // if (allResults[i] == null) { MAC0323
                        //     throw new NullPointerException("allMatches() " MAC0323
                        //            + "returned an array with a null entry"); MAC0323
                        // }
                        if (query == null) { // MAC0323
                            throw new NullPointerException("keysWithPrefix() "
                                   + "returned a null string");
                        }

                        // int tab = next.indexOf('\t'); MAC0323
                        // if (tab < 0) { MAC0323
                        //    throw new RuntimeException("allMatches() returned" MAC0323
                        //            + " an array with an entry without a tab:" MAC0323
                        //            + " '" + next + "'"); MAC0323
                        // } MAC0323
                        // String weight = next.substring(0, tab).trim(); MAC0323
                        // String query  = next.substring(tab);
                        long weight = auto.get(query); // MAC0323
                        
                        // truncate length if needed
                        if (query.length() > suggListLen.length())
                            query = query.substring(0, suggListLen.length());

                        // StdOut.println("i = " + i + "query =" + query + " weight = " + weight); MAC0323
                        // create the table HTML 
                        results[i] = "<html><table width=\"" 
                            + searchText.getPreferredSize().width + "\">"
                            + "<tr><td align=left>" 
                            // + query.substring(0, textLen + 1); // MAC0323
                            + query.substring(0, textLen); // MAC0323
                            // + "<b>" + query.substring(textLen + 1) + "</b>"; // MAC0323
                        if (query.length() > textLen) // MAC0323 
                            results[i] += "<b>" + query.substring(textLen) + "</b>"; // MAC0323
                        if (displayWeights) {
                        ...
                        i += 1; // MAC0323
                        if (i == resultsSize) break; // MAC0323
                    }
